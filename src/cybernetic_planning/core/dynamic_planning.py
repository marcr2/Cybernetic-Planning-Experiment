...
class DynamicPlanner:
    ...
    def update_capital_stock(
        self,
        year: int,
        investment_vector: np.ndarray,
        new_capital_stock: Optional[np.ndarray] = None
    ) -> None:
        """
    ...
    def create_five_year_plan(
        self,
        consumption_demands: List[np.ndarray],
        investment_demands: List[np.ndarray],
        use_optimization: bool = True
    ...
    def calculate_growth_rates(self) -> Dict[str, np.ndarray]:
        ...
        for i in range(1, len(years)):
            ...
            # Output growth
            output_growth = (
                self.plans[year]['total_output'] / 
                self.plans[prev_year]['total_output'] - 1
            )
            
            # Labor cost growth
            labor_growth = (
                self.plans[year]['total_labor_cost'] / 
                self.plans[prev_year]['total_labor_cost'] - 1
            )
            
            # Capital stock growth
            capital_growth = (
                self.plans[year]['capital_stock'] / 
                self.plans[prev_year]['capital_stock'] - 1
            )
