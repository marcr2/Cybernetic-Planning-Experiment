...
class ResourceDataCollector:
    ...
    def __init__(self, 
        ...
        self.scrapers = {
            ...
            'usgs': USGSScraper(cache_dir=str(self.cache_dir)),
            'bls': BLSScraper(cache_dir=str(self.cache_dir)),
            'epa': EPAScraper(cache_dir=str(self.cache_dir))
        }
    ...
    def print_api_key_setup_instructions(self) -> None:
        ...
        if self.api_manager:
            ...
            print("API key manager not available. Please set environment variables manually:")
            print("EIA_API_KEY=your_api_key_here")
            print("USGS_API_KEY=your_api_key_here")
            print("BLS_API_KEY=your_api_key_here")
            print("EPA_API_KEY=your_api_key_here")
    ...
    def collect_all_resource_data(self, year: int = 2024) -> Dict[str, Any]:
        ...
        """
        print(f"Collecting resource data for year {year}...")
        ...
        for source_name, scraper in self.scrapers.items():
            print(f"Collecting {source_name.upper()} data...")
            ...
            try:
                ...
                elif source_name == 'epa':
                    data = scraper.scrape_all_environmental_data(year)
                    all_data['environmental_data'] = data
                
                self.metadata['data_sources'].append(source_name)
                print(f"✓ {source_name.upper()} data collected successfully")
                
            except Exception as e:
                print(f"✗ Error collecting {source_name.upper()} data: {e}")
                continue
        ...
        print(f"Resource data collection completed for {year}")
        return all_data
    ...
    def _combine_resource_matrices(self, matrices: Dict[str, np.ndarray]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            print(f"Error combining resource matrices: {e}")
            return None
    ...
    def _save_collected_data(self, data: Dict[str, Any]) -> None:
        ...
        print(f"Data saved to {complete_file}")
        if 'resource_matrices' in data:
            print(f"Matrices saved to {matrices_file}")
    ...
    def get_collection_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'scrapers_status': {
                name: scraper.get_collection_summary() 
