...
class DataValidator:
    ...
    def _initialize_validation_rules(self) -> None:
        ...
        self.validation_rules = {
            ...
            'resource_matrix': {
                'must_be_non_negative': True,
                'must_have_positive_elements': True
            },
            'max_resources': {
                'must_be_non_negative': True,
                'must_have_positive_elements': True
    ...
    def validate_technology_matrix(self, matrix: np.ndarray, 
        ...
        if max_diagonal > self.validation_rules['technology_matrix']['diagonal_elements_max']:
            results['warnings'].append(
                f"Maximum diagonal element ({max_diagonal:.4f}) exceeds recommended threshold"
            )
        ...
        try:
            ...
            if spectral_radius >= 1.0:
                results['errors'].append(
                    f"Spectral radius ({spectral_radius:.4f}) >= 1, economy is not productive"
                )
                results['valid'] = False
    ...
    def validate_final_demand(self, vector: np.ndarray, 
                            strict: bool = False) -> Dict[str, Any]:
        """
    ...
    def validate_labor_input(self, vector: np.ndarray, 
                           strict: bool = False) -> Dict[str, Any]:
        """
    ...
    def validate_resource_matrix(self, matrix: np.ndarray, 
                               strict: bool = False) -> Dict[str, Any]:
        """
    ...
    def validate_max_resources(self, vector: np.ndarray, 
                             strict: bool = False) -> Dict[str, Any]:
        """
    ...
    def validate_all(self, data: Dict[str, Any], strict: bool = False) -> Dict[str, Any]:
        ...
        if 'final_demand' in data:
            results['component_results']['final_demand'] = self.validate_final_demand(
                data['final_demand'], strict
            )
        
        if 'labor_input' in data:
            results['component_results']['labor_input'] = self.validate_labor_input(
                data['labor_input'], strict
            )
        ...
        if 'max_resources' in data:
            results['component_results']['max_resources'] = self.validate_max_resources(
                data['max_resources'], strict
            )
