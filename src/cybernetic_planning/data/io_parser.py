...
class IOParser:
    ...
    def parse_custom_format(self, file_path: Union[str, Path], 
        ...
        data = {
            'name': config.get('name', 'custom_format'),
            'sectors': df.index.tolist(),
            'sector_count': len(df)
        }
    ...
    def validate_io_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'labor_input' in data:
            ...
            if len(labor_input) != tech_matrix.shape[0]:
                validation_results['errors'].append(
                    "Labor input vector length must match technology matrix dimensions"
                )
                validation_results['valid'] = False
    ...
    def _export_csv(self, data: Dict[str, Any], output_path: Path) -> None:
        ...
        df = pd.DataFrame(
            data['technology_matrix'],
            index=data['sectors'],
            columns=data['sectors']
        )
