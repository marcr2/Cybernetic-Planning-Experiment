...
def create_resource_utilization_chart(plan_data: Dict[str, Any], output_dir: str) -> str:
    ...
    for bar, util in zip(bars, utilization):
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width() / 2.0, height, f"{util:.1%}", ha="center", va="bottom")
...
def create_technology_heatmap(plan_data: Dict[str, Any], output_dir: str) -> str:
    ...
    # Create heatmap
    sns.heatmap(tech_matrix, annot = True, fmt=".3f", cmap="YlOrRd", cbar_kws={"label": "Input Coefficient"}, square = True)
...
def create_sensitivity_analysis_chart(sensitivity_data: Dict[str, Any], output_dir: str) -> str:
...
def create_growth_analysis_chart(growth_data: Dict[str, Any], output_dir: str) -> str:
