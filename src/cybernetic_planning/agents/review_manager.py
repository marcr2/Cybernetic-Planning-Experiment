    ...
    def _facilitate_cross_agent_communication(self, session: ReviewSession, selected_agents: List[str]):
        ...
        for agent_id, report in session.agent_reports.items():
            shared_context['agent_summaries'][agent_id] = {
                ...
                'main_risks': report.risk_assessment[:200] + "..." if len(report.risk_assessment) > 200 else report.risk_assessment
            }
            
            # Collect recommendations
            shared_context['recommendations'].extend([
                {'agent': agent_id, 'recommendation': rec} 
...
"""
    ...
    def _calculate_agent_consensus(self, agent_reports: Dict[str, AgentReport]) -> Dict[str, float]:
        ...
        if confidence_values:
            ...
            consensus['variance'] = sum((x - consensus['overall']) ** 2 for x in confidence_values) / len(confidence_values)
        
        return consensus
    
    def _prioritize_recommendations(self, agent_reports: Dict[str, AgentReport], synthesis_sections: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        for agent_id, report in agent_reports.items():
            for i, rec in enumerate(report.recommendations):
                all_recommendations.append({
                    ...
                    'priority_score': report.confidence_level * (len(report.recommendations) - i) / len(report.recommendations)
                })
    ...
    def _generate_implementation_roadmap(self, synthesis_sections: Dict[str, str], agent_reports: Dict[str, AgentReport]) -> List[Dict[str, Any]]:
...
"""
        ...
        for i, rec in enumerate(review.prioritized_recommendations, 1):
            text_report += f"{i}. {rec['recommendation']} (by {rec['agent_name']}, confidence: {rec['agent_confidence']:.2f})\n"
