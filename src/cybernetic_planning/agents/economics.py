...
class EconomicsAgent(BaseAgent):
    ...
    def _identify_critical_sectors(self, sensitivity_A: np.ndarray, sensitivity_d: np.ndarray) -> List[Dict[str, Any]]:
        ...
        for i in range(n):
            ...
            if importance_score > np.percentile([np.sum(np.abs(sensitivity_A[j, :, :])) + 
                                              np.sum(np.abs(sensitivity_d[j, :])) 
                                              for j in range(n)], 75):
    ...
    def _calculate_elasticities(self, A: np.ndarray, d: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            return {
                'output_elasticities': output_elasticities,
                'sector_elasticities': sector_elasticities
            }
            ...
            for j in range(n):
                if i != j and substitution_matrix[i, j] > 0.7:
                    substitution_opportunities.append({
                        'sector_i': i,
                        'sector_j': j,
                        'substitution_potential': substitution_matrix[i, j]
                    })
